...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select KROV Outputs to Spend" :
            "Select Shielded KROV to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::KROV;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show KROVACOIN address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
