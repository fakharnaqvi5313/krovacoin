...
#include "qt/krovacoin/krovacoingui.h"
...
#include "qt/krovacoin/defaultdialog.h"
#include "shutdown.h"
...
const QString KROVACOINGUI::DEFAULT_WALLET = "~Default";

KROVACOINGUI::KROVACOINGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void KROVACOINGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &KROVACOINGUI::toggleHidden);
 ...
 */
void KROVACOINGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &KROVACOINGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &KROVACOINGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &KROVACOINGUI::showHide);
...
void KROVACOINGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void KROVACOINGUI::handleRestart(QStringList args)
{
...
void KROVACOINGUI::setClientModel(ClientModel* _clientModel)
{
...
void KROVACOINGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &KROVACOINGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &KROVACOINGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void KROVACOINGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void KROVACOINGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
...
void KROVACOINGUI::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &KROVACOINGUI::hide);
                e->ignore();
            } else if ((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized()) {
                QTimer::singleShot(0, this, &KROVACOINGUI::show);
                e->ignore();
            }
        }
    }
...
void KROVACOINGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void KROVACOINGUI::messageInfo(const QString& text)
{
...
void KROVACOINGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
bool KROVACOINGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
...
void KROVACOINGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void KROVACOINGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KROVACOINGUI::detectShutdown()
{
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
...
void KROVACOINGUI::goToDashboard()
{
...
void KROVACOINGUI::goToSend()
{
    showTop(sendWidget);
}

void KROVACOINGUI::goToAddresses()
{
...
void KROVACOINGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void KROVACOINGUI::goToReceive()
{
    showTop(receiveWidget);
}

void KROVACOINGUI::openNetworkMonitor()
{
    settingsWidget->openNetworkMonitor();
}

void KROVACOINGUI::showTop(QWidget* view)
{
...
void KROVACOINGUI::changeTheme(bool isLightTheme)
{
...
void KROVACOINGUI::resizeEvent(QResizeEvent* event)
{
    // Parent..
...
bool KROVACOINGUI::execDialog(QDialog *dialog, int xDiv, int yDiv)
{
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void KROVACOINGUI::showHide(bool show)
{
...
bool KROVACOINGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    connect(dashboard, &DashboardWidget::incomingTransaction, this, &KROVACOINGUI::incomingTransaction);

    return true;
}

bool KROVACOINGUI::setCurrentWallet(const QString& name)
{
    // Single wallet supported.
    return true;
}

void KROVACOINGUI::removeAllWallets()
{
    // Single wallet supported.
}

void KROVACOINGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
...
static bool ThreadSafeMessageBox(KROVACOINGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void KROVACOINGUI::subscribeToCoreSignals()
{
...
void KROVACOINGUI::unsubscribeFromCoreSignals()
{
