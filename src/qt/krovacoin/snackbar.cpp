...
#include "qt/krovacoin/forms/ui_snackbar.h"
#include "qt/krovacoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(KROVACOINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &KROVACOINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
