...
#include "qt/krovacoin/dashboardwidget.h"
#include "qt/krovacoin/forms/ui_dashboardwidget.h"
#include "qt/krovacoin/sendconfirmdialog.h"
#include "qt/krovacoin/txrow.h"
...
DashboardWidget::DashboardWidget(KROVACOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquarePiv, "square-chart-krov");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelPiv, "text-chart-krov");
    ...
    setCssProperty(ui->labelAmountPiv, "text-stake-krov-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &KROVACOINGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("KROV")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair KROV, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            krov = (pair.first != 0) ? pair.first / 100000000 : 0;
        ...
        chartData->valuesPiv.append(krov);
        chartData->valuesMN.append(mn);

        int max = std::max(krov, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    if (chartData->totalPiv > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountPiv, "text-stake-krov");
