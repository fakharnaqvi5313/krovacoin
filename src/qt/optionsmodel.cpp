...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::KROV);
    ...
    if (settings.contains("nAnonymizeKrovacoinAmount") || reset)
        gArgs.SoftSetArg("-anonymizekrovacoinamount", settings.value("nAnonymizeKrovacoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in krovacoin.cpp during shutdown to also remove the window positions
