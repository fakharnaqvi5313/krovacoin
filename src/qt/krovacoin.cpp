...
#include "qt/krovacoin/splash.h"
#include "qt/krovacoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("krovacoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in krovacoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in krovacoin.qrc)
...
/** Class encapsulating KROVACOIN Core startup and shutdown.
...
/** Main KROVACOIN application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (KROVACOINGUI)
...
{
    window = new KROVACOINGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &KROVACOINGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &KROVACOINGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        // KROVACOIN: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &KROVACOINGUI::handlePaymentRequest);
        connect(window, &KROVACOINGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. KROVACOIN can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(krovacoin_locale);
    Q_INIT_RESOURCE(krovacoin);
    ...
    /// 6. Determine availability of data directory and parse krovacoin.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", KROVACOIN_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // krovacoin: links repeatedly have their payment requests routed to this process:
