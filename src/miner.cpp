...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("KROVACOINMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("KROVACOINMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("KROVACOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("krovacoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running KROVACOINMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("KROVACOINMiner exception");
    } catch (...) {
        LogPrintf("KROVACOINMiner exception");
    }

    LogPrintf("KROVACOINMiner exiting\n");
}
