...
#if defined(NDEBUG)
#error "KROVACOIN cannot be compiled without assertions."
#endif
...
DisconnectResult DisconnectBlock(CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        // not coinbases, they don't have traditional inputs
        if (tx.IsCoinBase())
            continue;
...
{
    util::ThreadRename("krovacoin-scriptch");
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    // KrovaCoin has no masternode reward and no coldstake-embedded budget
    // payments (see tasks 6/7 -- the Staking Rewards Pool pays out via its
    // own dedicated transaction, not a coinstake's free output), so a
    // differing last output here is never legitimate.
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // Cold Staking free-output enforcement
    ...
    if (pindexPrev != nullptr && block.hashPrevBlock != UINT256_ZERO) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            ...
            LogPrintf("%s: Cold-stake free-output check skipped on sync\n", __func__);
        }
    }
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Krovacoin Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nKrovacoinBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nKrovacoinBadBlockBits)) {
            // accept KROVACOIN block minted with incorrect proof of work threshold
            return true;
        }
