...
UniValue getinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total krovacoin balance of the wallet\n"
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"krovacoinaddress\"\n"
            "\nReturn information about the given krovacoin address.\n"

            "\nArguments:\n"
            "1. \"krovacoinaddress\"     (string, required) The krovacoin address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"krovacoinaddress\",    (string) The krovacoin address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address -only if is standard address-\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for KROVACOIN cold staking -only if is standard address-\n"
...
#ifdef ENABLE_WALLET
        // Case 1: KROVACOIN address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are krovacoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) krovacoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"krovacoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"krovacoinaddress\"  (string, required) The krovacoin address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "krova-cli and the GUI. There is no server-side difference."
        );
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ --------
    ...
    { "util",               "validateaddress",        &validateaddress,        true,  {"krovacoinaddress"} }, /* uses wallet if enabled */
    { "util",               "verifymessage",          &verifymessage,          true,  {"krovacoinaddress","signature","message"} },
