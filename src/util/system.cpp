...
const char * const KROVACOIN_CONF_FILENAME = "krovacoin.conf";
const char * const KROVACOIN_MASTERNODE_CONF_FILENAME = "masternode.conf";


// KROVACOIN only features
// Masternode
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "krovad -foo=bar
...
#else
    const char* pszModule = "krovacoin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\KROVACOIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\KROVACOIN
// Mac: ~/Library/Application Support/KROVACOIN
// Unix: ~/.krovacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KROVACOIN";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KROVACOINParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KROVACOINParams
    // Mac: ~/Library/Application Support/KROVACOINParams
    // Unix: ~/.krovacoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KROVACOINParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == nullptr || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "KROVACOINParams";
#else
    // Unix
    return pathRet / ".krovacoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/krovacoin/sapling-spend.params";
        sapling_output = "/usr/share/krovacoin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/krovacoin/sapling-spend.params";
            sapling_output = "/usr/local/share/krovacoin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", KROVACOIN_MASTERNODE_CONF_FILENAME));
