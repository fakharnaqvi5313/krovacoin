...
*Setup instructions for a gitian build of KROVACOIN Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the KROVACOIN
...
to the KROVACOIN GitHub release page.
...
- [Building KROVACOIN Core](#building-krovacoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/krovacoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/krovacoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/krovacoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/krovacoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/krovacoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/krovacoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/krovacoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@krovacoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/krovacoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/krovacoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/krovacoin-osx-*-build.assert
```
