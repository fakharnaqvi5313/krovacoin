KROVACOIN Core version *v5.0.0* is now available from:  <https://github.com/krovacoin-project/krovacoin/releases>
...
KROVACOIN Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Sapling Parameters
In order to run, KROVACOIN Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run KROVACOIN v5.0.0:
...
More visual information about the protocol can be found at https://krovacoin.org .
...
* Top bar showing shield KROV balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield KROV balance.
...
See the release notes from [v4.2.0](https://github.com/KROVACOIN-Project/KROVACOIN/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed KROV supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in KROV
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in KROV
...
* `getinfo`
  ...
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield KROV)
  The `moneysupply` JSON return entry is the total KROV supply (the sum of all transparent and shield KROV)
...
| `getaccountaddress` | Returned the current KROVACOIN address associated with an internal account | Account system removal |
| `setaccount` | Associate a KROVACOIN address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given KROVACOIN address | Account system removal |
| `getaddressesbyaccount` | Returned a list of KROVACOIN addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a KROVACOIN address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a KROVACOIN address | Pending a full rewrite |
...
| `spendzerocoin` | Spend zPIV to a KROV address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zPIV mints to a KROV address | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_krova unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake krovad and unit tests: fixing missing MacOS framework. (furszy)
