KROVACOIN Core version *3.3.0* is now available from:  <https://github.com/krovacoin-project/krovacoin/releases>
...
Moving forward from this undesired situation, we are enabling a secure and chain storage friendly solution for the zerocoin public spend (aka zPIV to KROV conversion).
...
The `masternode` and `mnbudget` RPC commands, which were marked as deprecated in KROVACOIN Core v2.3.1 (September 19, 2017), have now been completely removed from KROVACOIN Core.

Several new commands were added in v2.3.1 to replace the two aforementioned commands, reference the [v2.3.1 Release Notes](https://github.com/KROVACOIN-Project/KROVACOIN/blob/master/doc/release-notes/release-notes-2.3.1.md#rpc-changes) for further details.
