...
This document provides guidelines for internationalization of the KROVACOIN Core software.
...
This adds `<numerusform>`s to the respective `.ts` file, which can be translated separately depending on the language. In English, this is simply:
    ...
    <message numerus="yes">
        <source>%n active connection(s) to KROVACOIN network</source>
        <translation>
            <numerusform>%n active connection to KROVACOIN network</numerusform>
            <numerusform>%n active connections to KROVACOIN network</numerusform>
        </translation>
    </message>
...
This can be checked by executing `make translate` in the `src` directory, then verifying that `krovacoin_en.ts` remains unchanged.
