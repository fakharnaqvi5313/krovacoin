...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/KROVACOINParams"
    else
        PARAMS_DIR="$HOME/.krovacoin-params"
    fi
fi
...
function install_params {
    ...
    if ! [ -f "$filename" ]
    then
        filename="share/krovacoin/$filename"
    fi
...
function main() {
    ...
    cat <<EOF
KROVACOIN - install-params.sh

This script will install the KROVACOIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common KROVACOIN zkSNARK parameters. Note that it is
